...
{
    HAL_CRYP_SetKey(&hcryp, key);
    HAL_CRYP_Decrypt(&hcryp, in, 16, out, HAL_MAX_DELAY);
}

/* SHA-256 software implementation (FIPS 180-4). Self-contained, no
 * dependency on a hardware HASH peripheral, so it works the same way
 * on both the Secure Boot and Bootloader stages. */
...
void CRYPTO_SHA256(const uint8_t *data, uint32_t len, uint8_t *hash)
{
     ...
     * big-endian bit length. Needs a second block if the remainder plus
     * the 1-byte marker doesn't leave room for the 8-byte length field. */
